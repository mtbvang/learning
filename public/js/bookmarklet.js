...
var CryptoJS = CryptoJS || function(i, m) {
    var p = {}, h = p.lib = {}, n = h.Base = function() {
        ...
        return {extend: function(b) {
    ...
    }(), o = h.WordArray = n.extend({init: function(a, b) {
            a = 
            this.words = a || [];
            ...
            return o.create(b, a)
        }}), q = p.enc = {}, e = q.Hex = {stringify: function(a) {
        ...
        }}, g = q.Latin1 = {stringify: function(a) {
            for (var b = 
            a.words, a = a.sigBytes, c = [], d = 0; d < a; d++)
        ...
        }}, k = h.BufferedBlockAlgorithm = n.extend({reset: function() {
            this._data = o.create();
            this._nDataBytes = 0
        },_append: function(a) {
            ...
            var b = this._data, c = b.words, d = b.sigBytes, f = this.blockSize, e = d / (4 * f), e = a ? i.ceil(e) : i.max((e | 0) - this._minBufferSize, 0), a = e * f, d = i.min(4 * a, d);
            if (a) {
                for (var g = 0; g < a; g += f)
    ...
    h.Hasher = k.extend({init: function() {
            this.reset()
        },
        reset: function() {
            k.reset.call(this);
...
(function() {
    var i = CryptoJS, m = i.lib, p = m.WordArray, m = m.Hasher, h = [], n = i.algo.SHA1 = m.extend({_doReset: function() {
...
(function() {
    ...
    //
    // polyfill Object.keys based on http://tokenposts.blogspot.com.au/2012/04/javascript-objectkeys-browser.html
    // linked from https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/keys
    //
    if (!Object.keys) {
    ...
    var ACT_TYPE = "http://activitystrea.ms/schema/1.0/page", 
    RECIPE_ACT_ID = "http://id.tincanapi.com/recipe/bookmarklet/base/1", 
    RECIPE_ACT_TYPE = "http://id.tincanapi.com/activitytype/recipe", 
    TAG_PREFIX = "http://id.tincanapi.com/activity/tags/", 
    TAG_ACT_TYPE = "http://id.tincanapi.com/activitytype/tag", 
    QUALITY_RATING_EXT = "http://id.tincanapi.com/extension/quality-rating", 
    STATE_TAGS = "tags", 
    DEFAULT_PREV_RECORDS_CONTENT = "No past statements.", 
    bookmarkletActivity, 
    lrs, 
    statementCfg, 
    statement, 
    ...
    activity, 
    // one per load of app
    registration = TinCan.Utils.getUUID(), 
    opacity = 0.00, 
    contentSectionStyle = "padding-top: 10px;", 
     = {
        cache: "overlay",
        tag: "div",
        attrs: {
            ...
            style: [
                "position: fixed;", 
                "z-index: 2147483647;",  // top of 32 bit range
                "right: 10px;", 
                "top: 10px;", 
                "width: 300px;", 
                "margin: 0px;", 
        ...
        children: [
            {
                ...
                children: [
                    {tag: "span",attrs: {"class": "rbmTitle",style: "font-weight: bold;"},text: "Tin Can Reporter"}, 
                    {
                        cache: "closeBtn",
                        tag: "button",
                        attrs: {
            ...
            {
                tag: "div",
                attrs: {"class": "rbmContent",style: "padding: 0px 10px 7px 10px;"},
                children: [
                    {
                        tag: "div",
                    ...
                    }, 
                    {
                        ...
                        children: [
                            {tag: "label",text: "Verb: "}, 
                            {cache: "verbSelect",tag: "select",attrs: {"class": "rbmVerb"}}
                        ]
                    }, 
                    {
                        tag: "div",
                        ...
                        children: [
                            {tag: "label",text: "Rating: "}, 
                            {
                                ...
                                children: [
                                    ...
                                    {tag: "option",attrs: {value: "3",selected: "selected"},text: "3"}, 
                                    {tag: "option",attrs: {value: "4"},text: "4"}, 
                                    {tag: "option",attrs: {value: "5"},text: "5"}
                                ]
                            }
                        ]
                    }, 
                    ...
                    {
                        tag: "div",
                        attrs: {"class": "rbmContentSection",style: "padding: 10px; margin-top: 10px; background-color: #f5f5f5; border: 1px solid #ddd; font-size: 66%;"},
                        children: [
                            {tag: "div",attrs: {style: "font-weight: bold;"},text: "Previous Records"}, 
                            {cache: "previousRecords",tag: "div",attrs: {"class": "rbmPreviousRecords"},text: "Loading..."}
                        ]
                    }, 
                    {
                        tag: "div",
    ...
    bookmarkletActivity = new TinCan.Activity(
    {
        id: RECIPE_ACT_ID,
        definition: {
            type: RECIPE_ACT_TYPE
        }
    }
    );
    lrs = new TinCan.LRS(
    ...
    function buildStructure(cfg, parent) {
        var element = document.createElement(cfg.tag), 
        k, 
        i;
        
        if (typeof cfg.cache !== "undefined") {
            elements[cfg.cache] = element;
        }
        
        if (typeof cfg.attrs !== "undefined") {
            for (k in cfg.attrs) {
                if (cfg.attrs.hasOwnProperty(k)) {
                    element.setAttribute(k, cfg.attrs[k]);
                }
            }
        }
        
        if (typeof cfg.text !== "undefined") {
            element.appendChild(document.createTextNode(cfg.text));
        }
        
        if (typeof cfg.children !== "undefined") {
            for (i = 0; i < cfg.children.length; i += 1) {
                buildStructure(cfg.children[i], element);
            }
        }
    ...
    function buildVerbs() {
        ...
        for (i = 0; i < verbs.length; i += 1) {
            option = document.createElement("option");
            option.value = verbs[i].id;
    ...
    function setActivityId() {
        var parsedUrl = TinCan.Utils.parseURL(document.URL), 
        id, 
        prop, 
        params, 
        i;
        for (i = 0; i < blacklistedQueryParams.length; i += 1) {
            if (typeof parsedUrl.params[blacklistedQueryParams[i]] !== "undefined") {
                removedQueryParams[blacklistedQueryParams[i]] = parsedUrl.params[blacklistedQueryParams[i]];
                delete parsedUrl.params[blacklistedQueryParams[i]];
            }
        }
        ...
        if (Object.keys(parsedUrl.params).length > 0) {
            id += "?";
            params = [];
            for (prop in parsedUrl.params) {
                if (parsedUrl.params.hasOwnProperty(prop)) {
                    params.push(prop + "=" + encodeURIComponent(parsedUrl.params[prop]));
                }
            }
            id += params.join("&");
        }
    ...
    function setMsg(msg) {
        elements.msg.firstChild.data = msg;
    }
    ...
    function sendStatement() {
        ...
        statement, 
        {
            callback: function(err, xhr) {
                ...
                prevRecords = elements.previousRecords.innerHTML;
                if (prevRecords === DEFAULT_PREV_RECORDS_CONTENT) {
                    prevRecords = renderStatement(statement);
                } 
                else {
                    prevRecords = renderStatement(statement) + "<br>" + prevRecords;
                }
                
                elements.previousRecords.innerHTML = prevRecords;
    ...
    function previewStatement() {
        ...
        statementCfg.verb = {
            id: elements.verbSelect.value,
            display: {
                "en-US": elements.verbSelect.options[elements.verbSelect.selectedIndex].text
            }
        };
        
        statementCfg.context.extensions[QUALITY_RATING_EXT] = {
            min: 1,
            max: 5,
        ...
        if (tags !== "") {
            tags = tags.split(/[\s,]+/);
            if (tags.length > 0) {
                statementCfg.context.contextActivities.other = [];
                
                for (i = 0; i < tags.length; i += 1) {
                    statementCfg.context.contextActivities.other.push(
                    {
                        id: TAG_PREFIX + encodeURIComponent(tags[i]),
                        definition: {
                            type: TAG_ACT_TYPE,
                            name: {
                                "und": tags[i]
        ...
        statement = new TinCan.Statement(statementCfg, {doStamp: false});
        
        elements.previewStatement.firstChild.data = renderStatement(statement);
    }
    ...
    function renderStatement(st) {
        var result = [
            st.actor.toString(), 
            " ", 
            st.verb.toString("und"), 
            ' "', 
            st.target.toString("en-US"), 
            '" and rated it ', 
            st.context.extensions[QUALITY_RATING_EXT].raw, 
            ' out of 5'
        ].join(""), 
        i, 
        x;
        ...
        if (st.timestamp !== null) {
            result += " at " + st.timestamp;
        }
        result += ".";
        
        if (st.context.contextActivities.other !== null && st.context.contextActivities.other.length > 0) {
            result += " (";
            for (i = 0; i < st.context.contextActivities.other.length; i += 1) {
                x = st.context.contextActivities.other[i];
                if (x.definition.type === TAG_ACT_TYPE) {
                    if (i > 0) {
                        result += " ";
                    }
                    result += x.toString("und");
                }
            }
            result += ")";
        }
    ...
    function loadAgentTags() {
        ...
        {
            agent: actor,
            activity: bookmarkletActivity,
            callback: function(err, response) {
                if (err !== null) {
                    elements.tags.firstChild.data = "Error loading tags";
                    return;
                }
                if (response === null) {
                    elements.tags.firstChild.data = "<none>";
                    return;
                }
                
                agentTags = response.contents;
                agentTagsSHA1 = response.etag;
    ...
    function saveAgentTags() {
        //
        // do this ourselves so that we can calculate our own SHA1 for future send
        //
        var newContent = JSON.stringify(agentTags);
        ...
        newContent, 
        {
            ...
            callback: function(err) {
                ...
                agentTagsSHA1 = TinCan.Utils.getSHA1String(newContent);
    ...
    function displayAgentTags() {
        ...
        for (i = 0; i < tags.length; i += 1) {
            tagsHTML += ' <a href="' + tags[i] + '" class="rbmTag">' + tags[i] + '</a>';
        }
    ...
    function selectTag(newTag) {
        var currentText = elements.tagInput.value, 
        currentTags = currentText.split(/[\s,]+/), 
        uniqTags = [], 
        tagsMap = {}, 
        i;
        ...
        for (i = 0; i < currentTags.length; i += 1) {
            if (currentTags[i] !== "" && typeof tagsMap[currentTags[i]] === "undefined") {
                uniqTags.push(currentTags[i]);
                tagsMap[currentTags[i]] = true;
            }
        }
        uniqTags.sort();
        
        elements.tagInput.value = uniqTags.join(" ");
        
        previewStatement();
        setMsg("");
    }
    
    function loadPrevStatements() {
        // FEATURE: make it recursive, handling more links, etc.
        lrs.queryStatements(
        {
            params: {
                agent: actor,
            ...
            callback: function(err, stResult) {
                var i, 
                result = DEFAULT_PREV_RECORDS_CONTENT;
                
                if (err !== null) {
                    elements.previousRecords.innerHTML = "Can't load past statements: " + err;
                    
                    return;
                }
                
                if (stResult.statements.length > 0) {
                    result = "";
                    for (i = 0; i < stResult.statements.length; i += 1) {
                        result += renderStatement(stResult.statements[i]);
                        result += '<br>';
                    }
                }
                
                elements.previousRecords.innerHTML = result;
            }
        }
        );
    }
